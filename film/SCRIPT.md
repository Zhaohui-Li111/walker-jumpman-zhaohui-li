...
# Narration script 鈥?walker-jumpman-zhaohui-li explainer

Draft for the Brutalist `godot-walkthrough` workflow, `walker` modifier.
Beats 1, 15鈥?8 (Walker opening, Walker summary, Verdict, Your Turn, outro) are **owned by the
skill's template** and are not drafted here 鈥?the skill supplies their wording. What follows is
the body.
...
### 2 鈥?What this is  *(over the starter zone, then live play)*
...
### 3 鈥?The character  *(contact sheet on screen)*
...
body when you turn 鈥?so at eighteen pixels wide you can tell which way it is facing from the
...
### 4 鈥?Cause and effect: the collider did not change  *(code, then receipts)*
...
### 5 鈥?The extension  *(live play)*

The level was nine hundred and sixty pixels wide and finished at nine-sixteen. It is now sixteen
hundred wide and finishes at fifteen forty-eight 鈥?so the original route, on its own, can no
...
### 6 鈥?The decision  *(live play, low road)*
...
at speed 鈥?and then a sixty-four pixel pit before the finish pad. Flat targets, wide landings,
two real ways to die.

### 7 鈥?The other line  *(live play, high line)*

Or take the **high line**. A step, then two ledges. No hazard on it at all 鈥?but three jumps onto
...
if you change your mind 鈥?a fork that punishes curiosity is a bad fork.

### 8 鈥?Failure and recovery  *(live play, real death)*

Miss the pit and you get this. No lives, no menu 鈥?about half a second and you are back at the
start.
...
### 9 鈥?Cause and effect: the bug this film exists to explain  *(first draft, trace, current build)*
...
paper. The automated route for the high line passed. The low line died 鈥?on the spikes 鈥?even
...
character 鈥?I moved the high line up onto an intermediate step, to two-sixteen, which leaves
...
it and documented every new jump mark 鈥?I did not widen the tick budget or drop the
zero-deaths requirement to get a green report.

### 12 鈥?What is uncertain

Three things I do not know.

The entry jump onto the fork pad has a take-off window of roughly forty-six pixels of run-up.
That is **calculated, not measured** 鈥?it is the tightest input in the level and it is exactly
...
render. I caught it by looking at a screenshot 鈥?every test passed.

*(Render-day insert: the current, truthful status of the human playtest.)*

### 13 鈥?One next improvement

A check that measures the **width** of the take-off window at each new landing 鈥?sweep the jump
